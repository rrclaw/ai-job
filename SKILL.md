---
name: ai-job
description: >
  AI 时代求职闭环 skill：从头部 AI 公司人才观出发，走「JD 解析 → 素材挖掘 → 定位叙事 →
  简历生成（中英）→ 批判审阅（多 agent 对抗 + 代码闸）→ 面试准备 → 投递执行（邮箱全自动/
  门户半自动/直聊辅助）→ 跟踪复盘」八阶段闭环。
  当用户说"改简历 / 写简历 / 投递 XX 公司 / 求职 / 面试准备 / 自动投递 / ai-job"时使用。
---

# ai-job：AI 时代求职闭环

## 顶层思想

传统简历逻辑是「证明你合格」；AI 公司筛选逻辑是「证明你有 agency」。
详见 `references/talent-philosophy.md`（头部 AI 公司人才观调研）。三条公理贯穿全流程：

1. **可验证 > 可描述。** 一个能点开的链接胜过十行形容词。作品集（公开 repo、公网站点、
   可追溯数据）是简历的一等公民，不是附录。
2. **口径即诚信。** 每个数字必须带口径（时间窗、样本、统计方式），能被面试官当场验证。
   写不出口径的数字不许上简历。
3. **人味 > 完美。** AI 初筛之后是人终审。过度打磨的通用话术（中文「赋能/深耕/助力」，
   英文 spearheaded/passionate 滥用）是减分项。规则见 `references/anti-ai-flavor.md`。

## 八阶段闭环

每个候选人一个目录：`candidates/<name>/`（**默认 gitignore，个人数据永不入公开库**）。

### ① JD 解析与岗位画像
- 收集目标岗位 JD 原文（截图 OCR / 网页 / 招聘 app 转述），存 `candidates/<name>/jd/<公司>-<岗位>.md`
- 每份 JD 提炼：硬性门槛 / 核心职责 / 隐含偏好（从「为什么加入我们」段落反推）/ 关键词表（供 checks.py 覆盖率检查）
- 多家投递时做岗位聚类：一版主简历 + 每类岗位一个微调版，不做 N 份全定制

### ② 素材挖掘（evidence）
- 产出 `candidates/<name>/evidence.md`：分类罗列全部可用素材，**每条带来源和口径**
- 素材优先级：可公开验证的作品（链接）> 有第三方口径的成绩（平台数据）> 内部可追溯的工作成果（周报/报告）> 自述
- 红线：写简历时数字只能从 evidence.md 抄，禁止凭记忆写数
- **Agent 使用强度实测**（AI 岗强证据）：扫描本机 agent 会话日志（如 `~/.claude/projects/**/*.jsonl`），
  统计近 30 天的会话数、人工指令数、工具调用数、人机杠杆比、token 吞吐（含/不含缓存两种口径都留）、
  连续在线天数、最长连续时长。别人说自己会用 agent，你有电表读数
- 候选人自述（性格标签、成绩主张）也入 evidence，标注「本人自述」；锋利表述同时准备一个面试可防御版
- 时间敏感事实（排名/头衔/融资额）按**当时**口径核实，不用最新值：在读期间的学校排名比今年的更诚实也可能更好看
- 业绩类数字只用可验证的最强**真实**口径，禁止挑最佳子窗口（面试官一对原始数据就穿）；
  若成绩来自逐日锁档的前向记录，明写「非事后回测：凭证先落档、只能前向累计、任意一天可回放」，这句比数字本身更值钱

### ③ 定位与叙事
- 人岗匹配矩阵：JD 要求 × 素材证据，逐条对齐，找出「唯一性交集」（别人没有、岗位需要、你有证据的东西）
- 定一条主线叙事（一句话），全简历所有 bullet 都为主线服务；与主线无关的素材砍掉，再好也砍
- 产出 `candidates/<name>/positioning.md`

### ④ 简历生成
- 用 `templates/resume_cn.md` / `resume_en.md` 骨架，写法规则见 `references/methodology.md`（STAR/XYZ、量化 bullet、中英差异）
- 格式铁律：无照片、单栏、无表格排版、≤2 页、衬线中文/无衬线英文正文、联系方式+作品集链接置顶
- **标签行**（可选但强烈推荐）：摘要顶部一行加粗个人标签（4-6 个，「｜」分隔），选法=候选人自述×行为数据佐证，
  每个标签要么带括号数据、要么勾连简历内可验证事实；这是招聘官刷 100 份简历后唯一能背出来的一行
- 承认 AI 协作：大代码量必须写「与 XX 协作开发，本人负责架构/数据契约/质量闸」，在 AI 公司隐瞒 AI 参与是反向操作
- 多版本策略：主版本 + 每类岗位微调版（如产品岗把项目区提到工作经历前、砍证书装饰音）；核心数字各版本必须一致
- 渲染：`python3 scripts/render_resume.py <resume.md> --pdf --docx`，渲染后**必须打开 PDF 目检**（转义/分页/孤行）

### ⑤ 批判审阅（闭环的核心）
先跑代码闸，再跑对抗审阅，都过才算定稿：
- 代码闸：`python3 scripts/checks.py <resume.md> --jd <jd.md> --evidence <evidence.md> --links`
  （页数 / 量化密度 / AI 味词表 / JD 关键词覆盖率 / 链接可达）
- 对抗审阅：派 4 个独立 agent 并行，每个一个人设、只挑刺不夸：
  a. **资深 HR/HRD**（12 年阅历，通审全部版本：第一屏、结构密度、用力过猛、时间线硬伤、中英一致性；每版给 1-10 分）
  b. **目标岗技术/业务面试官**（按岗位换人设，只关心真实性：术语是否外行硬凑、规模宣称信不信、看完第一个想问什么、哪句会判虚标）
  c. **ATS + LLM 初筛器**（按 ats-and-ai-screening：先做 PDF 解析回测验证格式，再对每份 JD 给 0-100 匹配分 + 命中/缺失关键词 + 最优投递配对）
  d. **事实核查员**（逐数字对 evidence.md，主体错配/口径缺失/自评奖项一律打回）
- 审阅结论分 BLOCK / FLAG / PASS，BLOCK 必须改，FLAG 给用户拍板
- **拿到审阅后先分堆再改，不照单全收**：真硬伤（无口径、主体错配、逻辑断层）必改；风格/流量密码（大胆标签、反直觉表述）尊重候选人取舍，agent 的保守本能不等于对；已实测排除的（如某格式隐患已跑 pdftotext 验证干净）直接标注不动
- 高信息量表述配「安全绳」而非删除：一个反直觉的强数字（超高收益/超大规模/怪异纪律）后面紧跟一句口径或机制解释，流量与可信度同时拿到。例：策略高收益后加「样本尚短仅作 sanity」；"三次结果一致" 后加「知识世界与现实世界隔离、纯函数复现、非 LLM 实时调用」
- 无法核实来源的数字删掉，不硬撑（宁缺勿假）
- 实战验证过的高频 BLOCK（审阅 agent 必查）：
  1. **链接自爆**：点开作品集链接看到的东西 ≠ 文字宣称的规模 → 缩宣称或换链接，未公开部分明写「未开源，面试可演示」
  2. **预言无闭环**：「提示了/预判了」类 bullet 答不出「提示给谁、之后发生了什么」→ 补闭环或删
  3. **数字无口径**：任何收益/增速/排名没写时间窗和统计方式 → 补口径或删
  4. **自评奖项**：自建体系里给自己发的评级/最高档 → 删掉，只留可复核的原始数字
  5. **主体错配**：数字对但挂错了主语（平台/公司/产品张冠李戴）→ 事实核查员逐条对 evidence
- 判断错误不回避：被自己后续实践证伪的判断，写成「判断→证伪→更新」的认知迭代故事，比一直正确更有说服力

### ⑥ 面试准备
- 每个头部项目一张深挖卡：背景 / 关键决策 / 取舍 / 失败与证伪 / 如果重来
- 岗位题库（从 JD 职责反推面试问题）+ 认知题（该公司创始人/高管公开观点的应对）
- 反问清单（体现 agency 的问题，不问福利）+ 90 秒自我介绍稿（中英）

### ⑦ 投递执行（自动化分级，详见 references/apply-channels.md）
- 产出 `candidates/<name>/apply_plan.md`：目标公司 × 渠道 × 简历版本 × 主题行的作战单，按审阅阶段的匹配分排优先级
- **A 档 邮箱直投**：自动生成邮件（公司要求的主题格式 + 三句话正文 + 简历 PDF 附件）→ 建草稿 → 候选人过目点发送。全流程唯一留给人的动作是最后一次点击
- **B 档 官方门户**（Moka/飞书招聘/北森）：browser automation 填表，候选人只做验证码登录和最终提交
- **C 档 直聊平台**（Boss/猎聘）：只生成 mini 简历开场白，人工粘贴。禁自动打招呼（封号 + 模板痕迹被 HR 侧 AI 识别）
- 铁律：精准投递禁海投、投前 30 分钟内重验作品集链接、同公司多岗位间隔 ≥48h、投完立刻回填 tracker

### ⑧ 跟踪与复盘
- `candidates/<name>/tracker.md`：投递日期 / 渠道 / 版本 / 状态 / 反馈
- 每次拒信或面试反馈回填 evidence 和 positioning，迭代下一版。闭环在这里闭上。

## 目录结构

```
ai-job/
  SKILL.md
  references/
    talent-philosophy.md      # 头部 AI 公司人才观（定期刷新）
    methodology.md            # STAR/PAR/XYZ、量化 bullet、中英差异
    ats-and-ai-screening.md   # ATS/AI 筛选机制与格式规范
    anti-ai-flavor.md         # 反 AI 味文风清单
    apply-channels.md         # 投递通道手册（邮箱/门户/直聊三档自动化分级）
  templates/
    resume_cn.md  resume_en.md  resume.css
  scripts/
    render_resume.py          # md → PDF(headless chrome) + docx(python-docx)
    checks.py                 # 五项代码闸
  candidates/
    example/                  # 脱敏示例（虚构人物，演示七阶段产物）
    <name>/                   # 真实候选人，gitignore，永不入库
```

## 脱敏红线（公开库纪律）

- `candidates/*` 除 `example/` 外全部 gitignore；commit 前 `git status` 确认无个人文件
- references/templates/scripts 里不出现任何真实姓名、电话、邮箱、雇主、内部数据
- example 用虚构人物「林深 / Lin Shen」，数据一律标注「示例数据」
